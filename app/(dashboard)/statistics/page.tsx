    ...
    <div className="p-4 bg-background min-h-full">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
        <div className="flex items-center mb-4 md:mb-0">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">Analyze your portfolio and market trends</p>
          </div>
        </div>

        <div className="flex space-x-2">
          <button className="px-4 py-2 bg-card border border-border rounded-md text-sm font-medium flex items-center hover:bg-muted/50 text-foreground">
      ...
      <div className="bg-card rounded-lg border border-border p-4 mb-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex items-center space-x-2">
            <div className="flex space-x-1">
              <button
                className={`px-2 py-1 text-xs rounded ${chartType === "line" ? "bg-muted text-foreground" : "text-muted-foreground hover:bg-muted/50"}`}
                onClick={() => setChartType("line")}
              >
                Line
              </button>
              <button
                className={`px-2 py-1 text-xs rounded ${chartType === "bar" ? "bg-muted text-foreground" : "text-muted-foreground hover:bg-muted/50"}`}
                onClick={() => setChartType("bar")}
              >
                Bar
              </button>
              <button
                className={`px-2 py-1 text-xs rounded ${chartType === "candle" ? "bg-muted text-foreground" : "text-muted-foreground hover:bg-muted/50"}`}
            ...
            <div className="flex space-x-1">
              {["24H", "1W", "1M", "3M", "1Y", "ALL"].map((period) => (
                <button
                  key={period}
                  className={`px-2 py-1 text-xs rounded ${timeframe === period ? "bg-muted text-foreground" : "text-muted-foreground hover:bg-muted/50"}`}
        ...
        <div className="h-80 w-full">
          ...
          <svg className="w-full h-full" viewBox="0 0 800 300">
            {/* X and Y axes */}
            <line x1="50" y1="250" x2="750" y2="250" stroke="hsl(var(--border))" strokeWidth="1" />
            <line x1="50" y1="50" x2="50" y2="250" stroke="hsl(var(--border))" strokeWidth="1" />
            ...
            {[0, 20000, 40000, 60000, 80000].map((value, i) => (
              <text key={i} x="40" y={250 - i * 50} textAnchor="end" fontSize="10" fill="hsl(var(--muted-foreground))">
            ...
            <text x="660" y="35" fontSize="12" fill="hsl(var(--muted-foreground))">
              Bitcoin (BTC)
            </text>
            <circle cx="650" cy="50" r="5" fill="#3b82f6" />
            <text x="660" y="55" fontSize="12" fill="hsl(var(--muted-foreground))">
              Ethereum (ETH)
            </text>
          </svg>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-card rounded-lg border border-border p-4">
          <h2 className="text-lg font-bold mb-4 text-foreground">Top Gainers (24h)</h2>
                  ...
                  <tr key={index} className="border-b border-border last:border-0">
                    <td className="py-3">
                      <div className="flex items-center">
                        ...
                        <div className="text-xs text-muted-foreground ml-2">{coin.symbol}</div>
                      </div>
                    </td>
                    <td className="py-3 text-right text-foreground">{coin.price}</td>
        ...
        {/* Top Losers */}
        <div className="bg-card rounded-lg border border-border p-4">
          <h2 className="text-lg font-bold mb-4 text-foreground">Top Losers (24h)</h2>
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead>
                <tr className="text-xs text-muted-foreground border-b border-border">
      ...
      <div className="bg-card rounded-lg border border-border p-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-bold text-foreground">Trading Volume</h2>
          <div className="flex items-center space-x-2">
            <button className="p-1 rounded-md hover:bg-muted border border-border">
        ...
        <div className="h-60 w-full">
          ...
          <svg className="w-full h-full" viewBox="0 0 800 200">
            {/* X and Y axes */}
            <line x1="50" y1="150" x2="750" y2="150" stroke="hsl(var(--border))" strokeWidth="1" />
            ...
            {[0, 25, 50, 75, 100].map((value, i) => (
              <text key={i} x="40" y={150 - i * 30} textAnchor="end" fontSize="10" fill="hsl(var(--muted-foreground))">
                ${value}B
              </text>
            ))}
